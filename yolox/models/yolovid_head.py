...
from MyUtils.utils import gen_randint, featureprocess


class YOLOVIDHEAD(nn.Module):
    def __init__(
            self,
            num_classes,
            width=1.0,
        ...
        print("add supple detector")
        self.cls_preds_supple = self.cls_preds
        self.cls_convs_supple = self.cls_convs
        self.reg_convs_supple = self.reg_convs
        self.reg_preds_supple = self.reg_preds
        self.obj_preds_supple = self.obj_preds
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, cls_convs_supple, reg_convs_supple, stride_this_level, x) in enumerate(
            ...
            if self.isVid:
                ...
                bs = x.shape[0]
                k_regions = 20
                nums_frames = 7 if bs > 7 else bs - 1
                gridnum = x.shape[2]
                length_f = x.shape[1]
                cls_x_s = x
                reg_x_s = x

                cls_feat_s = cls_convs_supple(cls_x_s)
                cls_output_s = self.cls_preds_supple[k](cls_feat_s)

                reg_feat_s = reg_convs_supple(reg_x_s)
